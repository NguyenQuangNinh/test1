...
public function checkToks(inputstring:String):void {
	...
	if(nextToken(pstring,ppos)){
		   ...
		   if(tokentype=="f2"){
			   ...
			   if(!(tokenvalue=="max") && !(tokenvalue=="min")){
				...
				callError("Unknown public functions at position " + fpos +".");
			   ...
			   if(!(fchar=="(")){
				...
				return;
				    ...
				    callError("Two arguments expected for public function at position " + ppos +".");
...
private function conOper(inputstring:String, char:String):String {
  ...
  var curpos:int;
  var leftoper:String;
  var rightoper:String;
  var leftmove:Number;
  var rightmove:Number;
...
private function callError(mess:String):void {
	  ...
	  iserror = 1;
	  Utility.log("formula error " + errorMes);
...
private function checkEval(compiledExpression:Array):void {
	...
	for( var i:int = 0; i < compiledExpression.length; i++){
		...
		if(entrytype == "n"){
			...
			if(operands.length<1){callError("Check number of arguments in your public functions."); return;}

			operands.push(compiledExpression[i](operands.pop()))
		
		} else if( entrytype == "f2"){
			
			if(operands.length<2){callError("Check number of arguments in your public functions."); return;}
