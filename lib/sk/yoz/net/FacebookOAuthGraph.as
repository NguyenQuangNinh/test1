package sk.yoz.net
{
	...
	import com.playsoft.fwk.classes.OAuthManagement;
	import minigames.karagame.GlobalVariables;
    ...
    public class FacebookOAuthGraph extends EventDispatcher
    {
        ...
        public var clientSecret:String = "a14d9d0e2802456d624fa255203cba7b";
		...
		public function get expires():String { return _expires; }
        ...
        protected function verifyTokenSuccess(event:FacebookOAuthGraphEvent, token:String):void
        {
			Main.main.debugTrace("verifyTokenSuccess");
            _me = event.data;
            _token = token;
            _authorized = true;
        ...
        {
			Main.main.debugTrace("hash " + hash);
            if (hash)
			{
				_expires = hashToExpire(hash);
                verifyToken(hashToToken(hash));
			}
        }
		public function autoConnectWithSessionKey(fb_sig_session_key:String):void
		{
			var urlLoader:URLLoader = new URLLoader();
			urlLoader.addEventListener(Event.COMPLETE, getAccessTokenHdl);
			var req:URLRequest = new URLRequest("https://graph.facebook.com/oauth/exchange_sessions");
			req.method = "POST";
			var data:URLVariables = new URLVariables();
			data.client_id = clientId;
			data.client_secret = clientSecret;
			data.sessions = fb_sig_session_key;
			req.data = data;
			urlLoader.load(req);
		}
		
		private function getAccessTokenHdl(e:Event):void 
		{
			var accessToken:String = JSON.decode(URLLoader(e.currentTarget).data)[0].access_token;
			Main.main.debugTrace("accessToken == " + accessToken);
			
			autoConnect(accessToken);
		}
		
        public function autoConnect(session_token:String = null):URLLoader
        {
            if(session_token)
            {
                return verifyToken(session_token);
            }
		...
		protected function hashToExpire(hash:String):String
        {
            hash = hash.substr(0, 1) == "#" ? hash.substr(1) : hash;
            var data:URLVariables = new URLVariables(hash);
